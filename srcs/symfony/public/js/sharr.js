...
function schedSave() {
    stSave.textContent = 'Modification\u2026';
    clearTimeout(saveTimer);
    saveTimer = setTimeout(function () {
        stSave.textContent = 'Sauvegard\u00e9';
        setTimeout(function () { stSave.textContent = 'Sauvegarde auto'; }, 2000);
    }, 3000);
}
/* ══ LANGUAGE DETECTION ──────────════════════════════════════── */
...
function renderLines(code, curLine) {
    var lines = code.split('\n');
    var h = '';
    for (var i = 1; i <= lines.length; i++) {
        h += '<span class="ln' + (i === curLine ? ' cur' : '') + '">' + i + '</span>';
    }
    linesEl.innerHTML = h;
}
...
function getCur() {
    var b = input.value.substring(0, input.selectionStart).split('\n');
    return { line: b.length, col: b[b.length - 1].length + 1 };
}

function moveCaret() {
    var b  = input.value.substring(0, input.selectionStart).split('\n');
    var lh = parseFloat(getComputedStyle(input).lineHeight);
    var li = b.length - 1;

    var tmp = document.createElement('span');
    tmp.style.cssText = 'font:' + getComputedStyle(input).font + ';visibility:hidden;position:fixed;white-space:pre';
    tmp.textContent = b[li];
    document.body.appendChild(tmp);
    var lx = tmp.offsetWidth;
    document.body.removeChild(tmp);

    caretEl.style.left    = (18 + lx - input.scrollLeft) + 'px';
    caretEl.style.top     = (18 + li * lh - input.scrollTop) + 'px';
    caretEl.style.opacity = input.selectionStart !== input.selectionEnd ? '0' : '1';
}
...
function update() {
    ...
    var d    = detectLang(code.trim());

    if (d !== lang) {
        lang = d;
        stLang.textContent = d.charAt(0).toUpperCase() + d.slice(1);
    }

    output.innerHTML = render(tokenize(code, lang));
    var cur = getCur();
    renderLines(code, cur.line);
    stCursor.textContent = 'Ln ' + cur.line + ', Col ' + cur.col;
    stChars.textContent  = code.length + ' car.';
    moveCaret();
}

function schedUpdate() {
    if (!rafQ) { rafQ = true; requestAnimationFrame(update); }
}
...
input.addEventListener('keydown', function (e) {
    if (e.key === 'Tab') {
        ...
        var n = input.selectionEnd;
        input.value = input.value.substring(0, s) + '  ' + input.value.substring(n);
        input.selectionStart = input.selectionEnd = s + 2;
        schedUpdate();
    }
});

input.addEventListener('input',  function () { schedUpdate(); schedSave(); });
input.addEventListener('click',  schedUpdate);
input.addEventListener('keyup',  schedUpdate);
input.addEventListener('select', moveCaret);
